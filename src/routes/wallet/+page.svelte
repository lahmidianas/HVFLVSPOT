<script lang="ts">
  ...
  $: upcomingTickets = tickets.filter(
    (ticket) => ticket.events?.start_date && !isPast(new Date(ticket.events.start_date))
  );
  ...
  async function showQRCode(ticket: any) {
    ...
    try {
      const payload = JSON.stringify({
        ...
        ticket_id: ticket.tickets?.id,
        quantity: ticket.quantity
      });

      qrImageData = await QRCode.toDataURL(payload);
    } catch (err) {
      console.error('Failed to generate QR code', err);
      alert('Unable to generate QR code at the moment.');
      return;
    }
  ...
  function getTicketStatusColor(ticket: any) {
    // Past event always greyed
    if (ticket.events?.start_date && isPast(new Date(ticket.events.start_date))) {
      return 'bg-gray-100 text-gray-800';
    }

    // If QR redeemed, show as used
    if (ticket.qr_redeemed) {
      return 'bg-gray-100 text-gray-800';
    }
  ...
  function getTicketStatusText(ticket: any) {
    if (ticket.events?.start_date && isPast(new Date(ticket.events.start_date))) {
      return 'Past Event';
    }
...
<main class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
  ...
  <header class="mb-8 flex justify-between items-center">
    ...
    <button
      ...
      disabled={refreshing}
      class="bg-indigo-600 text-white px-4 py-2 rounded-md hover:bg-indigo-700 disabled:opacity-50 flex items-center space-x-2 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2"
  ...
  {:else if tickets.length === 0}
    ...
    <div class="text-center py-16">
      ...
      <a
        href="/events"
        class="inline-flex items-center bg-indigo-600 text-white px-6 py-3 rounded-md hover:bg-indigo-700 font-medium focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2"
      >
        <svg
          class="w-5 h-5 mr-2"
          fill="none"
      ...
      {#if upcomingTickets.length > 0}
        <section aria-labelledby="upcoming-heading">
          ...
          <div
            ...
            role="list"
            aria-label="Upcoming event tickets"
          >
              ...
              >
                ...
                <div class="relative h-48">
                  ...
                  <div class="absolute top-3 right-3">
                    <span
                      class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium {getTicketStatusColor(
                        ticket
                      )}"
                      role="status"
                    >
                ...
                <div class="p-6">
                  ...
                  <div
                    class="space-y-3 text-sm text-gray-600 mb-4"
                    role="list"
                  ...
                  >
                    ...
                    <div class="flex space-x-2">
                      <button
                        ...
                        class="bg-indigo-100 text-indigo-700 px-3 py-1 rounded text-sm hover:bg-indigo-200 font-medium focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2"
                      ...
                      <button
                        ...
                        disabled={ticket.qr_redeemed || !ticket.qr_code}
                        class="bg-green-100 text-green-700 px-3 py-1 rounded text-sm hover:bg-green-200 font-medium disabled:opacity-50 disabled:cursor-not-allowed focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2"
                        aria-label={`Validate ticket for ${ticket.events?.title}`}
                      >
                        {ticket.qr_redeemed ? 'Already Used' : 'Validate'}
                      </button>
                    </div>
                  </div>
                </div>
      ...
      {#if pastTickets.length > 0}
        <section aria-labelledby="past-heading">
          ...
          <div
            class="grid grid-cols-1 lg:grid-cols-2 gap-6"
            role="list"
            aria-label="Past event tickets"
          >
            {#each pastTickets as ticket (ticket.id)}
              <article
                class="bg-white rounded-lg shadow-md border border-gray-200 overflow-hidden opacity-75"
                role="listitem"
              >
                <!-- Event Image -->
                <div class="relative h-48">
                ...
                <div class="p-6">
                  <div class="mb-4">
                    <h3
                      class="text-lg font-semibold text-gray-900 mb-1"
                      id="past-ticket-{ticket.id}-title"
                    >
                      {ticket.events?.title}
                    </h3>
                    <p class="text-sm text-gray-600 line-clamp-2">
  ...
  >
    ...
    >
      <div class="text-center">
        ...
        <!-- QR Code -->
        <div
          class="bg-white border border-gray-200 rounded-lg p-4 mb-4 flex items-center justify-center"
          role="img"
          aria-label="QR code for ticket validation"
        >
          {#if qrImageData}
            <img src={qrImageData} alt="Ticket QR code" class="w-48 h-48 object-contain" />
          {:else}
            <div class="text-center text-gray-500 text-sm">Generating QR code...</div>
          {/if}
        </div>
        ...
        <div class="space-y-3">
          <button
            ...
            disabled={selectedTicket.qr_redeemed || !selectedTicket.qr_code}
            class="w-full bg-green-600 text-white px-4 py-2 rounded-md hover:bg-green-700 font-medium disabled:opacity-50 disabled:cursor-not-allowed focus:outline-none focus:ring-2 focus:ring-green-500 focus:ring-offset-2"
            aria-label="Validate this ticket"
          >
            {selectedTicket.qr_redeemed ? 'Ticket Already Used' : 'Validate Ticket'}
          </button>

          <button
            on:click={closeQRModal}
            class="w-full bg-gray-200 text-gray-700 px-4 py-2 rounded-md hover:bg-gray-300 font-medium focus:outline-none focus:ring-2 focus:ring-gray-500 focus:ring-offset-2"
          >
            Close
          </button>
        </div>
